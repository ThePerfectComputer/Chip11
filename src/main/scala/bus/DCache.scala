...
case class DataCacheMemBus()(implicit p : DataCacheConfig) extends Bundle with IMasterSlave{
  ...
  def toAxi4Shared() : Axi4Shared = {
    ...
    mm.sharedCmd.len := (cmd.length - 1).resized
    mm.sharedCmd.addr := cmd.address
    mm.sharedCmd.prot  := "110"
    mm.sharedCmd.cache := "1111"
    mm.sharedCmd.setBurstWRAP()
    mm.sharedCmd.write := cmd.wr

    mm.writeData.data := cmd.data
    rsp.data := 0
    mm.writeData.valid := False
    mm.writeData.last := False
    mm.writeData.strb := cmd.mask
    mm.readRsp.ready := True
    mm.writeRsp.ready := True


    val inBurst = Reg(Bool) init(false)
    val dataRemaining = Reg(UInt(log2Up(p.burstSize + 1) bits)) init(1)

    when(!cmd.wr){
      cmd.ready := mm.sharedCmd.ready
      rsp.valid := mm.readRsp.valid
      rsp.data := mm.readRsp.data
      mm.readRsp.ready := True
    }.otherwise {
      cmd.ready := True
      mm.sharedCmd.valid := False
      mm.sharedCmd.write := cmd.valid & cmd.wr
      // Write transactions are sent as a burst of transactions to the
      // same address. We therefore have to track some state
      rsp.valid := mm.readRsp.valid
      rsp.data := mm.readRsp.data
      mm.readRsp.ready := True
      when(!inBurst){
        ...
        when(cmd.valid) {
          mm.writeData.data := cmd.data
          mm.writeData.valid := cmd.valid
        }
        when(mm.sharedCmd.valid && mm.sharedCmd.ready){
          when(cmd.length > 1){
            inBurst := True
            dataRemaining := cmd.length - 2
          }.otherwise {
            mm.writeData.last := True
          }
        }
      }.otherwise {
        cmd.ready := mm.writeData.ready
        mm.sharedCmd.valid := False
        
        mm.writeData.data := cmd.data
        mm.writeData.valid := cmd.valid
        when(mm.writeData.valid && mm.writeData.ready){
          when(dataRemaining =/= 0){
