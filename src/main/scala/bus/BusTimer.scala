...
import spinal.lib.bus.amba4.axi.{Axi4, Axi4Shared, Axi4SlaveFactory, Axi4Config, Axi4CrossbarFactory}
...
class BusTimerShared(width: Int)(implicit config: Axi4Config) extends Component {
  val io = new Bundle {
    val bus = slave(Axi4Shared(config))
    val timerAMatch = out Bool
    val timerBMatch = out Bool
  }
